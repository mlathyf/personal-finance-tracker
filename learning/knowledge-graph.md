...
| Client-server request/response cycle (fetch → HTTP → route → response) | understood | 2026-09-14 | 2026-09-15 | Correctly predicted `app.listen`'s effect on process lifetime, then independently reasoned through *why* `curl` showed only `index.html` and not CSS/JS by asking the right question unprompted ("shouldn't it print the css and js file too") — after one explanation, correctly predicted and confirmed via two more `curl` calls (`style.css`, then `script.js` unprompted) that each static file is its own separate request/response, not bundled. |
...
| HTTP methods & REST conventions (GET/POST/DELETE mapping to CRUD) | introduced | 2026-09-15 | 2026-09-15 | Not yet exercised directly (no routes written besides static serving), but the request/response mechanics underlying REST (one URL → one response, `curl` vs. browser) were explained and confirmed via the `express.static` experiment above. |
| Express routing & middleware (`app.get`/`post`/`delete`, `express.json()`) | practicing | 2026-09-15 | 2026-09-15 | First wrote `app.get(express.static("public"))` (wrong — `app.get` with one arg is the settings getter, not a route/middleware registration); after the `app.use` vs `app.get` distinction was explained, independently corrected it to `app.use(express.static("public"))` in the actual file. Also wrote `app.listen(3000, callback)` correctly first try, with a working confirmation log in the callback. |
